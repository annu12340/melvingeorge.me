...
### ✅ Note: Another cool thing is that now if we check the contents of the `john` object, you can see the value we added even though we added the property directly to the proxy object.

So we added validation in our `set` function.

Whew 😅. That's a lot. Let's define our `get` function in the next blog post ✌️.
